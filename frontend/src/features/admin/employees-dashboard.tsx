...
const initialEmployees: Employee[] = [
  ...
  },
  {
    ...
    role: "expert",
    category: "травля и оскорбления",
    login: "a.petrova",
    rating: 4.9,
  },
  {
    id: 4,
    fullName: "Соколова М.А.",
    role: "expert",
  ...
  {
    id: 6,
    fullName: "Воронов И.М.",
    role: "expert",
    category: "юридический вопрос",
    login: "i.voronov",
    rating: 4.7,
  },
];
  ...
  return (
    <DialogShell labelledBy="new-employee-heading" onClose={onClose} showClose className="max-w-[750px]">
      <form onSubmit={handleSubmit} noValidate>
        <h2 id="new-employee-heading" className="pr-12 text-[28px] leading-tight font-extrabold text-[#4562f0] max-[499px]:text-[23px]">
          Новый сотрудник
        </h2>
        ...
        <div className="mt-6 grid gap-5">
          <label className="grid gap-2 text-base font-medium text-[#151515]">
            ...
            <Input
              ...
              autoComplete="name"
              aria-required="true"
              className="!h-[42px] !w-full !rounded-[12px] !bg-[#fcfdff] !px-3 !text-left !text-sm focus:!bg-white focus:!text-[#000828]"
            />
          </label>
          ...
          <label className="grid gap-2 text-base font-medium text-[#151515]">
            ...
            <select
              value={role}
              onChange={(event) => handleRoleChange(event.target.value as EmployeeRole | "")}
              required
              aria-required="true"
          ...
          <label className="grid gap-2 text-base font-medium text-[#151515]">
            ...
            <select
              ...
              aria-required={role === "expert"}
              aria-describedby="category-help"
              className={controlClass}
            >
              <option value="" disabled>
                {role === "operator" ? "Операторам категория не назначается" : "Выберите категорию"}
              </option>
            ...
            <span id="category-help" className="text-xs font-normal text-[#646d86]">
              {role === "expert"
                ? "Категория определяет сферу работы эксперта."
                : role === "operator"
                  ? "Для роли оператора категория не используется."
                  : "Сначала выберите роль сотрудника."}
            </span>
          </label>
          ...
          <label className="grid gap-2 text-base font-medium text-[#151515]">
            ...
            <span className="relative block">
              <Input
                ...
                aria-required="true"
                className="!h-[42px] !w-full !rounded-[12px] !bg-[#fcfdff] !px-3 !pr-12 !text-left !text-sm focus:!bg-white focus:!text-[#000828]"
              />
              <button
                type="button"
                ...
                className="absolute top-1/2 right-2 flex h-9 w-9 -translate-y-1/2 cursor-pointer items-center justify-center rounded-lg text-[#646d86] hover:bg-[#eef1ff] hover:text-[#4562f0] focus-visible:outline-2 focus-visible:outline-offset-1 focus-visible:outline-[#4562f0]"
              >
...
export default function EmployeesDashboard() {
  ...
  const filteredEmployees = useMemo(() => {
    ...
    return employees.filter((employee) => {
      ...
      const matchesSearch =
        !query ||
        employee.fullName.toLocaleLowerCase("ru").includes(query) ||
        employee.login.toLocaleLowerCase("ru").includes(query) ||
        employee.category?.toLocaleLowerCase("ru").includes(query);
  ...
  function addEmployee(employee: NewEmployee) {
    setEmployees((current) => [
      ...current,
      { ...employee, id: Math.max(0, ...current.map((item) => item.id)) + 1 },
    ]);
  ...
  return (
    <section className="min-w-0 flex-1 px-5 pt-5 pb-8 sm:px-[26px]" aria-labelledby="employees-heading">
      <div className="mx-auto w-full max-w-[1180px]">
        ...
        <h1 id="employees-heading" className="mt-7 text-[26px] leading-tight font-extrabold tracking-[-0.02em] text-[#4562f0]">
          Сотрудники
        </h1>
        ...
        <div className="mt-4 grid gap-4 md:grid-cols-2">
          <label className="relative block">
            ...
            <Input
              ...
              className="!h-[42px] !w-full !rounded-[12px] !border-[#4562f0] !bg-white !pr-3 !pl-[42px] !text-left !text-sm placeholder:!text-[#9196a7] focus:!bg-white focus:!text-[#000828]"
            />
          </label>
        ...
        <div className="mt-5 grid grid-cols-3" role="tablist" aria-label="Фильтр сотрудников по роли">
          {filters.map((item) => (
            <button
              ...
              className={`cursor-pointer border-b-2 px-3 py-3 text-sm transition-colors focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#4562f0] ${
                filter === item.value
                  ? "border-[#4562f0] font-medium text-[#4562f0]"
                  : "border-transparent text-[#30384f] hover:border-[#b5c0f9] hover:text-[#4562f0]"
              }`}
            >
              {item.label}
            </button>
        ...
        <section aria-label="Список сотрудников" className="mt-3 overflow-x-auto rounded-[13px] border border-[#4562f0] bg-white/85">
          <table className="w-full min-w-[760px] table-fixed border-collapse">
            <thead className="bg-[#dfe6ff] text-[#4562f0]">
              <tr className="h-[52px]">
                <th scope="col" className="w-[27%] border-r border-[#4562f0] px-4 text-center font-normal">Фамилия ИО</th>
                <th scope="col" className="w-[19%] border-r border-[#4562f0] px-4 text-center font-normal">Роль</th>
                <th scope="col" className="w-[29%] border-r border-[#4562f0] px-4 text-center font-normal">Категория</th>
                <th scope="col" className="w-[25%] px-4 text-center font-normal">Ссылка на аналитику</th>
              </tr>
            </thead>
            <tbody>
              {filteredEmployees.map((employee) => (
                <tr key={employee.id} className="h-[58px] border-t border-[#4562f0] hover:bg-[#f7f8ff]">
                  <td className="border-r border-[#4562f0] px-5 text-sm font-medium text-[#000828]">
                    {employee.fullName}
                    {employee.rating && (
                      <span className="ml-2 rounded-md bg-[#dfe6ff] px-1.5 py-0.5 text-[11px] font-normal text-[#4562f0]">{employee.rating.toFixed(1)}</span>
                    )}
                  </td>
                  ...
                  <td className="border-r border-[#4562f0] px-4 text-center text-sm text-[#30384f]">
                    {employee.role === "expert" && employee.category
                      ? employee.category.charAt(0).toUpperCase() + employee.category.slice(1)
                      : "—"}
                  </td>
                  <td className="px-4 text-center">
                    <Link href={`/admin/analytics?employee=${employee.id}`} className="inline-flex min-w-[105px] items-center justify-center rounded-full bg-[#4562f0] px-4 py-1 text-xs text-white transition-colors hover:bg-[#4f71fc] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#4562f0]">
                      Открыть
                    </Link>
                  </td>
                </tr>
              ...
              {filteredEmployees.length === 0 && (
                <tr className="h-28 border-t border-[#4562f0]">
                  <td colSpan={4} className="px-6 text-center text-sm text-[#646d86]">Сотрудники не найдены</td>
                </tr>
              )}
            </tbody>
          </table>
        ...
        <p className="mt-3 text-right text-xs text-[#7a8298]" aria-live="polite">
      ...
      {isModalOpen && (
        <AddEmployeeModal
          existingLogins={employees.map((employee) => employee.login)}
