...
const controlClass =
  "h-[38px] w-full rounded-[9px] border border-[#000828] bg-white px-3 text-sm text-[#000828] outline-none transition-colors hover:border-[#4562f0] focus:border-[#4562f0] focus:ring-2 focus:ring-[#4562f0]/20";
...
function QuestionSelectionModal({
  ...
  return (
    <DialogShell labelledBy="questions-modal-heading" onClose={onClose} showClose className="max-w-[720px]">
      <h2 id="questions-modal-heading" className="pr-12 text-[27px] leading-tight font-extrabold text-[#4562f0] max-[499px]:text-[22px]">
        Выбор уточняющих вопросов
      </h2>
      ...
      <section aria-labelledby="questions-list-heading" className="mt-5">
        ...
        <div className="mt-2 overflow-hidden rounded-[12px] border border-[#000828] bg-[#fcfdff]">
          <button
            ...
            className="flex min-h-11 w-full cursor-pointer items-center gap-3 border-b border-[#bdc2cf] px-4 text-left text-sm text-[#30384f] transition-colors hover:bg-[#eef1ff] hover:text-[#4562f0] focus-visible:outline-2 focus-visible:outline-inset focus-visible:outline-[#4562f0]"
          >
                ...
                <li key={question.id} className="flex min-h-[58px] items-center gap-3 border-b border-[#e1e4ec] px-4 last:border-b-0">
                  <label className="flex min-w-0 flex-1 cursor-pointer items-center gap-3 text-sm text-[#30384f]">
                    <input
                      type="checkbox"
                      checked={checked}
                      onChange={() => onToggle(category, question.id)}
                      className="h-4 w-4 shrink-0 accent-[#4562f0]"
                    />
                  ...
                  <button
                    ...
                    className="shrink-0 cursor-pointer rounded-[8px] bg-[#4562f0] px-4 py-2 text-xs text-white transition-colors hover:bg-[#4f71fc] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#4562f0]"
                  >
  ...
  return (
    <DialogShell labelledBy="edit-question-heading" onClose={onClose} showClose className="max-w-[670px]">
      <form onSubmit={handleSubmit} noValidate>
        <h2 id="edit-question-heading" className="pr-12 text-[27px] leading-tight font-extrabold text-[#4562f0] max-[499px]:text-[22px]">
          {question ? "Редактировать вопрос и ответы" : "Новый уточняющий вопрос"}
        </h2>
        ...
        <label className="mt-6 grid gap-2 text-base font-medium text-[#151515]">
          ...
          <Input
            ...
            required
            autoFocus
            className="!h-[42px] !w-full !rounded-[12px] !bg-white !px-3 !text-left !text-sm focus:!bg-white focus:!text-[#000828]"
          />
        </label>
        ...
        <section aria-labelledby="answers-heading" className="mt-5">
          <div className="flex items-center justify-between gap-4">
            ...
            <button
              ...
              className="cursor-pointer rounded-md px-2 py-1 text-sm text-[#4562f0] hover:bg-[#eef1ff] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#4562f0]"
            >
              + Добавить ответ
            </button>
          </div>
            ...
            {options.map((option, index) => (
              <div key={index} className="flex items-center gap-2">
                <Input
                  ...
                  className="!h-[38px] !w-full !rounded-[9px] !bg-white !px-3 !text-left !text-sm focus:!bg-white focus:!text-[#000828]"
                />
                <button
                  type="button"
                  ...
                  className="flex h-9 w-9 shrink-0 cursor-pointer items-center justify-center rounded-lg text-xl text-[#646d86] hover:bg-[#fff1f1] hover:text-[#d70d14] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#4562f0] disabled:pointer-events-none disabled:opacity-30"
                >
                  ×
                </button>
              </div>
...
export default function CategoriesDashboard() {
  ...
  function ensureCategoryConfig(category: string) {
    setConfigs((current) => {
      if (current.some((item) => item.category === category)) return current;
      return [
        ...current,
        {
  ...
  function addCategory() {
    ...
    ensureCategoryConfig(nextCategory);
    setSaved(false);
  }
  ...
  function toggleQuestion(category: string, questionId: string) {
    setConfigs((current) => {
      ...
      return current.map((item) => item.id === existing.id
        ? {
            ...item,
            questionIds: item.questionIds.includes(questionId)
              ? item.questionIds.filter((id) => id !== questionId)
              : [...item.questionIds, questionId],
          }
        : item);
    });
    setSaved(false);
  ...
  function saveQuestion(question: Omit<ClarifyingQuestion, "id">) {
    if (editingQuestionId) {
      ...
      setConfigs((current) => current.map((item) => item.category === questionModalCategory
        ? { ...item, questionIds: [...item.questionIds, id] }
        : item));
    }
  ...
  return (
    <section className="min-w-0 flex-1 px-5 pt-5 pb-8 sm:px-[26px]" aria-labelledby="categories-heading">
      <div className="mx-auto w-full max-w-[1180px]">
        <Link href="/admin" className="inline-flex rounded-sm text-sm text-[#85899b] hover:text-[#4562f0] focus-visible:outline-2 focus-visible:outline-offset-3 focus-visible:outline-[#4562f0]">
          Вернуться назад
        </Link>

        <h1 id="categories-heading" className="mt-7 text-[26px] leading-tight font-extrabold tracking-[-0.02em] text-[#4562f0]">
          Категории
        </h1>
        ...
        <section aria-label="Настройка категорий" className="mt-7 overflow-x-auto rounded-[13px] border border-[#4562f0] bg-white/85">
          <table className="w-full min-w-[760px] table-fixed border-collapse">
            <thead className="bg-[#dfe6ff] text-[#4562f0]">
              <tr className="h-[52px]">
                <th scope="col" className="w-[34%] border-r border-[#4562f0] px-4 text-center font-normal">Категория</th>
                <th scope="col" className="w-[32%] border-r border-[#4562f0] px-4 text-center font-normal">Специализация</th>
                <th scope="col" className="w-[34%] px-4 text-center font-normal">Список уточняющих вопросов</th>
              </tr>
            </thead>
            <tbody>
              {configs.map((config) => (
                <tr key={config.id} className="h-[62px] border-t border-[#4562f0] hover:bg-[#f7f8ff]">
                  <td className="border-r border-[#4562f0] px-3">
                    <select
                      ...
                      onChange={(event) => updateConfig(config.id, {
                        category: event.target.value,
                        specialist: defaultSpecialist(event.target.value),
                      })}
                      aria-label={`Категория строки ${config.id}`}
                      className={controlClass}
                    >
                      {adminCategories.map((category) => (
                        <option key={category} value={category} disabled={category !== config.category && usedCategories.has(category)}>
                  ...
                  <td className="border-r border-[#4562f0] px-3">
                    <select
                      value={config.specialist}
                      onChange={(event) => updateConfig(config.id, { specialist: event.target.value as SpecialistOption })}
                      aria-label={`Специализация для категории ${config.category}`}
                      className={controlClass}
                    >
                  ...
                  <td className="px-4 text-center">
                    <button
                      type="button"
                      onClick={() => setQuestionModalCategory(config.category)}
                      className="inline-flex min-w-[128px] cursor-pointer items-center justify-center rounded-[8px] bg-[#4562f0] px-4 py-2 text-xs text-white transition-colors hover:bg-[#4f71fc] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#4562f0]"
                    >
                      Выбрать ({config.questionIds.length})
                    </button>
                  </td>
                </tr>
              ))}
        ...
        <button
          ...
          className="mt-4 cursor-pointer rounded-md px-1 py-1 text-sm text-[#30384f] hover:text-[#4562f0] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#4562f0] disabled:cursor-default disabled:text-[#a0a6b7]"
        >
          + {nextCategory ? "Добавить ещё категорию" : "Все категории добавлены"}
        </button>
      ...
      {questionModalCategory && editingQuestionId === undefined && (
        <QuestionSelectionModal
          category={questionModalCategory}
          configs={configs}
          questions={questions}
          onCategoryChange={(category) => {
            ensureCategoryConfig(category);
            setQuestionModalCategory(category);
          }}
          onToggle={toggleQuestion}
          onEdit={(questionId) => setEditingQuestionId(questionId)}
          onAddQuestion={() => setEditingQuestionId(null)}
          onClose={() => setQuestionModalCategory(null)}
        />
      )}
      ...
      {questionModalCategory && editingQuestionId !== undefined && (
        <EditQuestionModal
          ...
          onSave={saveQuestion}
          onClose={() => setEditingQuestionId(undefined)}
        />
      )}
    </section>
  );
