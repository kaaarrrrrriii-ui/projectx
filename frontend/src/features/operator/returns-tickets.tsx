...
import { applicantLabels, returnedTickets, type TicketPriority } from "./tickets";
...
export default function ReturnsTickets() {
  ...
  const filteredTickets = useMemo(() => {
    ...
    return returnedTickets.filter((ticket) =>
  ...
  return (
    <section className="min-w-0 flex-1 px-5 pt-5 pb-0 sm:px-[26px]" aria-labelledby="returns-heading">
      <div className="mx-auto w-full max-w-[1180px]">
        <label className="relative block">
          ...
          <Input
            ...
            aria-label="Поиск возвращённого обращения по трек-номеру"
        ...
        <header className="mt-4">
          <h1 id="returns-heading" className="text-[26px] leading-tight font-extrabold tracking-[-0.02em] text-[#4562f0]">
            Возвраты
          </h1>
          <p className="mt-1 text-base text-[#151515]">Сначала самые ранние</p>
        </header>

        <section aria-label="Фильтры возвращённых обращений" className="mt-5">
        ...
        <section aria-label="Список возвращённых обращений" className="mt-3 overflow-x-auto rounded-[13px] border border-[#4562f0] bg-white/85">
            ...
            <tbody>
              ...
              {filteredTickets.length === 0 && (
                <tr className="h-24 border-t border-[#4562f0]">
                  <td colSpan={4} className="text-center text-sm text-[#646d86]">Возвраты не найдены</td>
                </tr>
              )}
